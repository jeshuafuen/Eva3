...
if (!nombre_resultado) {
  mostrarAlerta('Se necesita completar el campo.', 'danger');
  return;
}
...
fetch("http://144.126.210.74:8080/api/resultado", requestOptions)
  .then((response) => {
      if (response.status == 200) {
          mostrarModal('Resultado actualizado exitosamente.', 'success');
          setTimeout(() => {
              location.href = "listar.html";
          }, 2000);
      } else if (response.status == 400) {
          mostrarModal('Error al actualizar el resultado.', 'danger');
      }
  })
  .then((result) => console.log(result))
  .catch((error) => {
      console.error(error);
      mostrarModal('Error al actualizar el resultado: ' + error.message, 'danger');
  });
  }
  ...
  function eliminarResultado(){
    ...
    fetch("http://144.126.210.74:8080/api/resultado"+ g_id_resultado, requestOptions)
      .then((response) => {
        if (response.status == 200) {
          mostrarModal('Resultado eliminado exitosamente.', 'success');
          setTimeout(() => {
              location.href = "listar.html";
          }, 2000);
      }
