...
function actualizarCliente(){
  ...
  .then((response) => {
      if (response.status == 200) {
          mostrarModal('Cliente actualizado exitosamente.', 'success');
          setTimeout(() => {
              location.href = "listar.html";
          }, 2000);
      } else {
  ...
  .catch((error) => {
      console.error(error);
      mostrarModal('Error al actualizar el cliente: ' + error.message, 'danger');
  });
}
  ...
  function eliminarCliente(){
    ...
    fetch("http://144.126.210.74:8080/api/cliente/" + g_id_cliente, requestOptions)
        .then((response) => {
            if (response.status == 200) {
              mostrarModal('Cliente eliminado exitosamente.', 'success');
              setTimeout(() => {
                  location.href = "listar.html";
              }, 2000);
          }
