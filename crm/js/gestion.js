...
fetch("http://144.126.210.74:8080/api/gestion", requestOptions)
  .then((response) => {
      if (response.status == 200) {
          mostrarModal('Gestión agregada exitosamente.', 'success');
          setTimeout(() => {
              location.href = "listar.html";
          }, 2000);
      } else if (response.status == 400) {
          mostrarModal('Error al crear la gestión.', 'danger');
      }
  })
  .then((result) => console.log(result))
  .catch((error) => {
      console.error(error);
      mostrarModal('Error al crear la gestión: ' + error.message, 'danger');
  });
}
...
function completarEtiqueta(element,index,arr){
  ...
  document.getElementById('lbl_eliminar').innerHTML ="¿Desea eliminar gestión? <b>" + id_gestion + "</b>";
...
function completarFormulario(element,index,arr){
  ...
  document.getElementById('sel_id_resultado').value = id_tipo_gestion;
...
fetch("http://144.126.210.74:8080/api/gestion", requestOptions)
  .then((response) => {
      if (response.status == 200) {
          mostrarModal('Gestión actualizada exitosamente.', 'success');
          setTimeout(() => {
              location.href = "listar.html";
          }, 2000);
      } else if (response.status == 400) {
          mostrarModal('Error al actualizar la gestión.', 'danger');
      }
  })
  .then((result) => console.log(result))
  .catch((error) => {
  ...
  function eliminarGestion(){
    ...
    fetch("http://144.126.210.74:8080/api/gestion/"+ g_id_gestion, requestOptions)
      .then((response) => {
        if (response.status == 200) {
          mostrarModal('Gestion eliminada exitosamente.', 'success');
          setTimeout(() => {
              location.href = "listar.html";
          }, 2000);
      }
